...
(async function boot() {
  ...
  if (ok) {
    Map3D.setGround(ART.dir + 'ground-mandala.jpg');
    const weights = {};
...
function viewYatra() {
  ...
  openSheet('My Yatra', YATRA.length + ' stops', rows +
    ...
    `<div class="two" style="margin-top:12px">
       <button class="cta ghost" onclick="optimiseYatra()">🧭 Order my route</button>
       <button class="cta ghost" onclick="window.print()">🖨 Print / save as PDF</button>
     </div>`);
}
...
function startBackdrop() {
  const wrap = $('#backdrop');
  if (!wrap) return;
  // bundled illustrations first — they always load, even with no internet
  BG = ART.bg.map((b, i) => ({ url: ART.bgUrl(i), cap: b.c + ' · illustration' }));
  if (!BG.length) return;
  ...
  if (c) {
    ...
    body += `<section class="psec" id="s-fes"><h2>${esc(t('festivals'))}</h2>
      ...
      <div class="cards c3">` + c.festivals.map(f => `<article class="pc sm">
        ${photo(f.img, f.n, f.w, false, ART.match('festival', f.n))}
    ...
    body += `<section class="psec" id="s-food"><h2>${esc(t('food'))}</h2>
      ...
      <div class="cards c3">` + c.food.map(f => `<article class="pc sm">
        ${photo(f.img, f.n, '', false, ART.match('dish', f.n))}
    ...
    body += `<section class="psec" id="s-craft"><h2>${esc(t('crafts'))}</h2>
      ...
      <div class="strip">` + c.crafts.map((x, i) => `<article class="pc sm">
        ${photo((c.craftImgs || [])[i], x, '', false, ART.dir + ART.craftDefault)}
    ...
    /* immersive: VR + AR */
    body += `<div class="immersive">
      <button class="imbtn" onclick="openVR('${jsq(state)}','${jsq(district)}')">
        <span class="beta">360°</span><span class="ic">🥽</span>
        <b>Step inside in VR</b>
        <small>Look around ${esc(district)} in 360° — gyroscope and Cardboard supported</small></button>
      <button class="imbtn" onclick="AR.open('${jsq(state)}','${jsq(district)}')">
        <span class="beta">Beta</span><span class="ic">🧭</span>
        <b>Find it in AR</b>
        <small>Point your phone around you and see heritage sites with live bearing and distance</small></button>
    </div>`;

    /* getting there */
    if (c.transport) { add('s-go', 'Getting there'); body += transportHTML(c.transport, district); }
...
/* Regional illustration as the state banner, with the real photograph
   fading in on top of it when (and only when) it loads. */
function stateBanner(state, s) {
  const b = ART.banner(state);
...
function renderHome() {
  ...
  const byRegion = {};
  Object.keys(DISTRICTS).forEach(s => {
    const r = ART.stateRegion[s] || 'north';
  ...
  if (rg) rg.innerHTML = Object.keys(REGION_LABEL).map(r => {
    ...
    const b = ART.dir + ART.region[r];
    const alt = ART.dir + (ART.regionFallback[r] || 'bg-1.jpg');
...
function viewRegion(region) {
  const states = Object.keys(DISTRICTS).filter(s => (ART.stateRegion[s] || 'north') === region);
  if (!states.length) { go('#/'); return; }
  const b = ART.banner(states[0]);
...
/* ==================================================================
   GETTING THERE — airport, railhead, road, and door-to-door steps
   ================================================================== */
function transportHTML(tp, district) {
  const card = (cls, kind, name, code, km, note) => `
    <div class="trcard ${cls}">
      <div class="k">${esc(kind)}</div>
      <b>${esc(name)}</b>
      <div>${code && code !== '—' ? `<span class="code">${esc(code)}</span>` : ''}
        <span class="km">${km ? km + ' km away' : ''}</span></div>
      <p>${esc(note)}</p>
    </div>`;

  const air = tp.air || [], rail = tp.rail || [];
  return `
    <div class="sd">Door-to-door, from the airport and from the railway station.
      Distances and fares are approximate — confirm locally.</div>
    <div class="trgrid">
      ${card('', '✈ Nearest airport', air[0] || '—', air[1], air[2], air[3] || '')}
      ${card('rail', '🚆 Railhead', rail[0] || '—', rail[1], rail[2], rail[3] || '')}
      <div class="trcard road"><div class="k">🛣 By road</div>
        <b>Highways &amp; buses</b><p>${esc(tp.road || '')}</p></div>
    </div>

    <div class="pgrid2" style="margin-top:20px">
      <div>
        <h3 style="font-size:13px;letter-spacing:.18em;text-transform:uppercase;color:var(--green);
                   font-family:Inter,sans-serif;margin-bottom:10px">🚆 From the railway station</h3>
        <ol class="steps">${(tp.fromRail || []).map(x => `<li>${esc(x)}</li>`).join('')}</ol>
      </div>
      <div>
        <h3 style="font-size:13px;letter-spacing:.18em;text-transform:uppercase;color:var(--blue);
                   font-family:Inter,sans-serif;margin-bottom:10px">✈ From the airport</h3>
        <ol class="steps">${(tp.fromAir || []).map(x => `<li>${esc(x)}</li>`).join('')}</ol>
      </div>
    </div>

    <div style="margin-top:16px">
      <div class="k" style="font-size:10.5px;letter-spacing:.18em;text-transform:uppercase;color:var(--dim)">
        Getting around ${esc(district)}</div>
      <div class="fares">${(tp.local || []).map(x => `<span class="fare">${esc(x)}</span>`).join('')}</div>
    </div>
    ${tp.tip ? `<div class="note" style="margin-top:14px">💡 <b>Local tip:</b> ${esc(tp.tip)}</div>` : ''}
    ${tp.access ? `<div class="note" style="margin-top:8px">♿ <b>Accessibility:</b> ${esc(tp.access)}</div>` : ''}`;
}

function helplinesHTML() {
  return `<div class="helpline">
    <a href="tel:112"><b>112</b> · Emergency</a>
    <a href="tel:1363"><b>1363</b> · Tourist helpline</a>
    <a href="tel:139"><b>139</b> · Railway enquiry</a>
    <a href="tel:1091"><b>1091</b> · Women's helpline</a>
    <a href="tel:1926"><b>1926</b> · Forest / wildlife</a>
  </div>`;
}

/* ------------------------------------------------------------ VR open */
function openVR(state, district) {
  const c = CONTENT.districts[state + '|' + district];
  const spots = c ? c.monuments.slice(0, 4).map(m => m.n) : [];
  VR.open(state, district, spots);
}

/* ==================================================================
   NEAR ME — what heritage is around the visitor right now
   ================================================================== */
function nearMe() {
  if (!navigator.geolocation) { toast('Location is not available here', true); return; }
  toast('Finding heritage around you…');
  navigator.geolocation.getCurrentPosition(p => {
    const me = [p.coords.latitude, p.coords.longitude];
    const R = 6371, rad = d => d * Math.PI / 180;
    const list = Object.keys(CONTENT.districts).map(k => {
      const d = CONTENT.districts[k];
      if (!d.coords) return null;
      const dLat = rad(d.coords[0] - me[0]), dLon = rad(d.coords[1] - me[1]);
      const a = Math.sin(dLat / 2) ** 2 + Math.cos(rad(me[0])) * Math.cos(rad(d.coords[0])) * Math.sin(dLon / 2) ** 2;
      const km = 2 * R * Math.asin(Math.sqrt(a));
      const [s, di] = k.split('|');
      return { s, di, km, c: d };
    }).filter(Boolean).sort((a, b) => a.km - b.km).slice(0, 12);

    openSheet('Around you', `Nearest verified guides to your location`,
      `<div class="note" style="margin-bottom:12px">📍 ${me[0].toFixed(3)}, ${me[1].toFixed(3)} — your position stays on this device.</div>
       <div class="grid">` + list.map(x => `
        <div class="dcard withimg" onclick="go('#/d/${encodeURIComponent(x.s)}/${encodeURIComponent(x.di)}')">
          ${x.c.hero ? `<img class="thumb" loading="lazy" src="${esc(x.c.hero)}" alt="" onerror="this.remove()">` : ''}
          <b>${esc(x.di)}</b>
          <small>${x.km < 10 ? x.km.toFixed(1) : Math.round(x.km)} km · ${esc(x.s)}</small></div>`).join('') +
      `</div><button class="cta" style="margin-top:14px" onclick="AR.open()">🧭 Open the AR finder</button>`);
  }, () => toast('Location permission denied', true), { enableHighAccuracy: true, timeout: 12000 });
}

/* ==================================================================
   MY YATRA — order the stops so the trip actually makes sense
   ================================================================== */
function optimiseYatra() {
  const pts = YATRA.map(y => {
    const c = CONTENT.districts[y.s + '|' + y.d];
    return { y, co: c && c.coords };
  });
  if (pts.filter(p => p.co).length < 3) { toast('Add at least 3 mapped stops first', true); return; }
  const R = 6371, rad = d => d * Math.PI / 180;
  const dist = (a, b) => {
  ...
  const withCo = pts.filter(p => p.co), without = pts.filter(p => !p.co);
  const out = [withCo.shift()];
  while (withCo.length) {                       // nearest-neighbour ordering
    const last = out[out.length - 1].co;
    let bi = 0, bd = Infinity;
    withCo.forEach((p, i) => { const d = dist(last, p.co); if (d < bd) { bd = d; bi = i; } });
    out.push(withCo.splice(bi, 1)[0]);
  }
  let total = 0;
  for (let i = 1; i < out.length; i++) total += dist(out[i - 1].co, out[i].co);
  YATRA = out.concat(without).map(p => p.y);
  localStorage.setItem('by360.yatra', JSON.stringify(YATRA));
  viewYatra();
  toast('Route ordered — about ' + Math.round(total) + ' km of travel');
}
